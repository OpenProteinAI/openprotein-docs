...
function addDescription() {
  ...
  for (const groupName in HIERARCHY) {
    ...
    description.innerText = HIERARCHY[groupName].description;
    tag.parentElement.appendChild(description);
  }

  // add model descriptions
  const backendUrl = getBackendUrl();
  fetch(backendUrl + "api/v1/embeddings/models", {
    headers: {
      Authorization:
        "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MTI2MjY4MDcuMTI3ODY4LCJuYmYiOjE3MTE5MzU2MDcsInN1YiI6IjEiLCJ0eXBlIjoiYWNjZXNzX3Rva2VuIiwiaXNzIjoib3BlbnByb3RlaW4uYWkifQ.wZjk9SpA_XqI3ULtktkDv1dJXobrtYKi3_7RkT95K_I",
    },
  })
    .then((response) => response.json())
    .then((models) => {
      for (const model of models) {
        const tag = document.querySelector(`#operations-tag-${model}`);
        if (tag == null) {
          continue;
        }
        fetch(backendUrl + `api/v1/embeddings/models/${model}`, {
          headers: {
            Authorization:
              "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE3MTI2MjY4MDcuMTI3ODY4LCJuYmYiOjE3MTE5MzU2MDcsInN1YiI6IjEiLCJ0eXBlIjoiYWNjZXNzX3Rva2VuIiwiaXNzIjoib3BlbnByb3RlaW4uYWkifQ.wZjk9SpA_XqI3ULtktkDv1dJXobrtYKi3_7RkT95K_I",
          },
          ...
          .then((modelMetadata) => {
            const description = document.createElement("p");
            description.innerText = modelMetadata.description.summary;
            tag.parentElement.appendChild(description);
          });
      }
    });
